...
class suxingme_mostviews extends WP_Widget {
    ...
    function suxingme_mostviews() {
        $widget_ops = array('description' => '默认显示30天内，浏览次数最多的图文列表');
        $this->__construct('suxingme_mostviews', '热门浏览', $widget_ops);
    }
						...
						<a class="img" href="<?php the_permalink(); ?>"	title="<?php the_title(); ?>">
							<i class="icon-forward"></i>
								<?php if( suxingme('suxingme_timthumb_lazyload')) { ?>
									<img class="lazy thumbnail" data-original="<?php echo get_template_directory_uri(); ?>/timthumb.php?src=<?php echo post_thumbnail_src(); ?>&h=150&w=260&zc=1" src="<?php echo get_template_directory_uri(); ?>/timthumb.php?src=<?php echo constant("THUMB_SMALL_DEFAULT");?>&h=150&w=251&zc=1" alt="<?php the_title(); ?>" />
								<?php }elseif (suxingme('suxingme_timthumb')) {	?>
									<img src="<?php echo get_template_directory_uri(); ?>/timthumb.php?src=<?php echo post_thumbnail_src(); ?>&h=150&w=260&zc=1" alt="<?php the_title(); ?>" class="thumbnail"/>
								<?php } else { ?>
									<img src="<?php echo post_thumbnail_src(); ?>" alt="<?php the_title(); ?>" class="thumbnail" />
								<?php } ?>						
						</a>	
							<div class="title">
								<div class="entry-meta"><span><i class="icon-eye"></i> <?php post_views('',''); ?></span><span><i class="icon-calendar-1"></i> <?php the_time('Y-n-j') ?></span></div>
